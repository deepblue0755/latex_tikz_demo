...
\begin{tikzpicture}[node distance=10mm and 10mm]
  ...
  % --- row 4: right to left ---
  \node[ssys, soptional, below=of q8] (q9) {\sboxtext{9}{预编译模块矩阵}{可选，并入 payload}};
  \node[ssys, left=of q9] (q10) {\sboxtext{10}{打包源码}{生成 \texttt{.tar.gz}}};
...
\begin{tikzpicture}[node distance=10mm and 10mm]
  ...
  \node[ssys, right=of s1] (s2) {\sboxtext{2}{环境自检}{依赖检查、卸载驻留驱动}};
...
步骤 2 除依赖检查外还会卸载已驻留的驱动，是最早改动系统状态的一步。}
